...
function createNewTask(text, check) {
    ...
    // to clone upper visible li in the HTML page
    ...
    newLi.children[0].setAttribute("href", `#collapse${count}`); // identical collapseId

    newLi.children[0].classList.remove("line-throught");

    newLi.children[0].innerText = `${text} - Added on ${formattedDate}`; // including date and time
    if (check === true) {
    ...
    count++; // for a different value of collapseId
    mainInput.value = "";
}
...
function addInnerTask(upperText, mainText, check) {
    // to clone inner visible li from the HTML page
    ...
    // adding new inner task to the last of the list before form
    let tempList;
...
function taskStufs(e) {
    ...
    if (e.target.classList.contains("delete-item")) {
        ...
        if (e.target.parentElement.children[0].classList.contains("line-throught")) {
